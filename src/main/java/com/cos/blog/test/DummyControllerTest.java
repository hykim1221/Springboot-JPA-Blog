...
public class DummyControllerTest {
	...
	// email, password 수정 가능하게
	// 밑에건 get이고 이건 put이라 주소가 같아도 됨
	...
	public String delete(@PathVariable int id) {
		...
		try {
			...
			return "삭제실패했습니다. 해당아이디는 데이터베이스에 없습니다.";
		}
		return "삭제되었습니다. id: "+id;
	}
	
	@Transactional // Transaction을 걸면 save를 하지 않아도 된다. 함수종료시에 자동 commit이 된다.
	...
	// {id} 주소로 파라매터를 전달 받을 수 있음.
	// http://localhost:8000/blog/dummy/user/3
	@GetMapping("/dummy/user/{id}")
	public User detail(@PathVariable int id) {
		// user/4을 찾으면 내가 데이터베이스에서 못찾아오게 되면 user가 null이 된다
		// 프로그램에 문제가 발생하기 때문에 Optional로 너의 User객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return해라
		...
		// 요청 : 웹브라우저
		// user객체 : 자바 오브젝트
		// 변환 (웹브라우저가 이해할수있는데이터) ->json(json라이브러리)
